(function (Scratch) {
  ...
  class YouTubeBlocks {
    ...
    getInfo() {
      return {
        ...
        blocks: [
          ...
          '---',
          {
            opcode: 'buildSyncString',
            blockType: Scratch.BlockType.REPORTER,
            text: 'sync string for [URL] starting now',
            arguments: { URL: { type: Scratch.ArgumentType.STRING, defaultValue: 'https://youtu.be/dQw4w9WgXcQ' } }
          },
          {
            opcode: 'applySyncString',
            blockType: Scratch.BlockType.COMMAND,
            text: 'apply sync string [DATA]',
    ...
    buildSyncString(args) {
      return `${args.URL}|${Date.now()}`;
    }

    applySyncString(args) {
      const raw = String(args.DATA || '');
      const sepIndex = raw.lastIndexOf('|');
      if (sepIndex === -1) return;
      const url = raw.slice(0, sepIndex);
      const startedAt = Number(raw.slice(sepIndex + 1));
      if (!url || !Number.isFinite(startedAt)) return;
      ...
      this.yt.whenReady(() => {
        ...
        if (elapsed > 0.5) {
          this.yt.player.seekTo(elapsed, true);
        }
      });
    }
